...
template <typename T>
int BinaryTree<T>::sumDistances(Node* subroot, int distanceSoFar) const{
    if (subroot == NULL){
        return 0;
    }

    return sumDistances(subroot->left, distanceSoFar + 1 ) + sumDistances(subroot->right, distanceSoFar+1) + distanceSoFar;
}
