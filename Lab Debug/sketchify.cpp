...
HSLAPixel* myFavoriteColor(double saturation) {
    ...
    //We must instantiate a pointer to the HSLAPixel object
    HSLAPixel *pixel = new HSLAPixel(170, saturation, 0.5);

    // we cannot return &pixel because references 
    // allocates memory on the stack for the variable
    // as soon as function ends the stack memory is discarded
    // so if we return address to local variable; when function is over
    // the reference to the stack variable is gone
    // so we need to allocate memory on the heap by
...
void sketchify(std::string inputFile, std::string outputFile) {
    // Load in.png
    PNG* original = new PNG(); // it cannot be null!
    ...
    for (unsigned y = 1; y < height -1; y++) { //we have to make sure we don't go out of range so we do height -1
        for (unsigned x = 1; x < width- 1; x++) { // same with the width -1 here
            ...
            if (score > 0.3) {
                ...
                //We can also do it in one line ! :o
                // We dereference the pointer so we get the object and copy it to the myPixel object
